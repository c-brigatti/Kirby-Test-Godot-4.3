...
public partial class Battle : Control
{
	...
	[Signal]
	public delegate void NextEventEventHandler(BattleActor activeActor);
	...
	public void StartNextEvent()
	{
		...
		EmitSignal("NextEvent", _activeActor);
		...
		{
			var partyMembers = Data.Instance.PartyMembers;
			BattleActor randomPlayer = partyMembers[GD.RandRange(0, partyMembers.Count - 1)];
			Ability randomAbility;
			if (GD.RandRange(0, 1) == 0)
			{
				randomAbility = _activeActor.Skills[GD.RandRange(0, _activeActor.Skills.Count - 1)];
			}
			else
			{
				randomAbility = _activeActor.Magic[GD.RandRange(0, _activeActor.Magic.Count - 1)];
			}

			RunEnemyEvent(_activeActor, randomPlayer, randomAbility);
		}
	}

	public async void RunEnemyEvent(BattleActor enemy, BattleActor target, Ability ability)
	{
		// TODO: change wait timeout to simulate animation instead
		await ToSignal(GetTree().CreateTimer(0.25f), "timeout");
		
		GD.Print("Enemy is attacking " + target.Name);
		target.ChangeHp(-ability.Damage);
		enemy.ChangeMp(-ability.MpCost);
	...
	public void _on_next_event(BattleActor actor)
	{
		if (actor is PlayerActor)
		{
			foreach (var button in _players.GetButtons())
			{
				var playerButton = (PlayerInfoButton)button;
				if (playerButton.PlayerData == actor)
				{
